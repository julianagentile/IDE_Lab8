...
void Timer32_1_Init(void(*task)(void), unsigned long period, enum timer32divider div)
{
	...
	// timer reload value
	// TIMER32_LOAD1
	TIMER32_LOAD1 = timer1Period;   // Reload with the initial value for 0.5 sec
  
	
	// clear Timer32 Timer 1 interrupt
	// TIMER32_INTCLR1
	
	TIMER32_INTCLR1 |= BIT0;  // Stop Timer32-1
	...
	TIMER32_CONTROL1 |= (BIT7|BIT1|BIT6|BIT5);  
	TIMER32_CONTROL1 |= (div << 2); // set the clock divider
...
void Timer32_2_Init(void(*task)(void), unsigned long period, enum timer32divider div)
{
	...
	//TIMER32_CONTROL2   
  	TIMER32_CONTROL2 |= BIT7; // enable timer
	TIMER32_CONTROL2 |= BIT6; // periodic mode
	TIMER32_CONTROL2 |= BIT5; // enable interrupt
	TIMER32_CONTROL2 |= (div << 2); // set the clock divider
